...
jQuery(function($) {
    ...
    /*--------------------------------
         Custom Dropzone
     --------------------------------*/
    ULTRA_SETTINGS.cobrandingSpotcap = function() {
        ...
        if ($.isFunction($.fn.dropzone)) {
            ...
            var i = 1,
                $custom_droplist = $("#cobranding-droptable"),
                example_dropzone = $("#cobrandingDZ").dropzone({
                    url: 'data/upload-file.php',
                    maxFiles:1,
                    acceptedFiles: "image/*",
                    ...
                    addedfile: function(file) {
                        //if (i == 1) {
                            $custom_droplist.html('');
                            $("#fileinfo").html("Uploading...");
                            $("#uploadedlogo").attr("src","");                       
                            $("#iconindicator .fa-arrow-up").show();
                            $("#iconindicator .fa-check").hide();
                       ...
                       // size = size < 1024 ? (size + " KB") : (parseInt(size / 1024, 10) + " MB");
                        ...
                        var $el = $('<div class="progress"><div class="progress-bar progress-bar-warning"></div></div>');

                        $custom_droplist.append($el);
                        file.fileEntryTd = $el;
                        file.progressBar = $el.find('.progress-bar');
                    },

                    uploadprogress: function(file, progress, bytesSent) {
                        file.progressBar.height(progress + '%');
                        $("#iconindicator .percent").html(Math.round(progress) + '%');
                        $("#iconindicator .fa-arrow-up").show();
                        $("#iconindicator .fa-check").hide();
                        /*$('.cobranding-dropzone .drop-table').perfectScrollbar({
                            suppressScrollX: true
                        });*/
                    },
                    ...
                    success: function(file) {
                       ...
                       // if(response.html==undefined || response.html==null) {
                        ...
                        //}
                        file.progressBar.removeClass('progress-bar-warning').addClass('progress-bar-success');
                        $("#fileinfo").html(file.name+" ("+size+")");
                        $("#uploadedlogo").attr("src",file.xhr.responseText);
                        $("#iconindicator .fa-arrow-up").hide();
                        $("#iconindicator .fa-check").show();
                    },
...
//                        $this.find('span').html(start.format(drp.format) + drp.separator + end.format(drp.format));
    ...
    $(document).ready(function() {
        ...
        ULTRA_SETTINGS.cobrandingSpotcap();
    });
